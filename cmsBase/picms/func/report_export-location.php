...
$file = fopen("report-location.xls", "w");

// Escrever o cabeçalho do arquivo Excel (com BOM para UTF-8)
fwrite($file, chr(0xEF) . chr(0xBB) . chr(0xBF)); // UTF-8 BOM
fwrite($file, "Data;Regiao;UF;Cidade;Bairro;Acao;Aplicativo;Tipo;Template;Conteudo;Versao;Data Versao;Em Producao?;Dispositivo;Modelo\n");
...
do {
    ...
    $result = navigationControlByLocationList($offset, $limit); // Adaptar a função para aceitar offset e limit
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $line = $row["ActionDate"] . ";"
                . $row["Region"] . ";" . $row["State"] . ";" . $row["City"] . ";" . $row["Neighbor"] . ";"
