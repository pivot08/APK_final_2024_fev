...
function locationInfo() {
    ...
    this.getCities = function(id) {
        ...
        var method = "post";
        var data = {};
        jQuery('.cities').find("option:eq(0)").html("Por favor aguarde..");
        call.send(data, url, method, function(data) {
            jQuery('.cities').find("option:eq(0)").html("Selecione a cidade");
            if(data.tp == 1){
                if(data.hits > 1000000)
                {
                    //alert('Free usage far exceeded. Please subscribe at geodata.solutions.');
                    console.log('Daily geodata.solutions request limit exceeded count:' + data.hits + ' of 100000');
                }
                else
                {
                    console.log('Daily geodata.solutions request count:' + data.hits + ' of 100000')
                }
